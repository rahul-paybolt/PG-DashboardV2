// Required imports
import { setGAuthUser } from "@/store/global-store";
import {
...
export const initiateGoogleSignIn = (
  ...
  loadScript("https://accounts.google.com/gsi/client", () =>
    onGoogleAuthLoad(buttonRef, navigator, isSignIn)
  );
};

// Handle Google Auth Load
const onGoogleAuthLoad = (
  buttonRef: HTMLDivElement,
  navigator: Navigator,
  isSignIn: boolean
) => {
...
const onGoogleSignIn = async (
  googleCred: CredentialsResponse,
  navigator: Navigator
) => {
  const [response, error] = await loginWithGoogle({
    id_token: googleCred.credential,
    recaptcha: token,
  });
  if (error) return;
